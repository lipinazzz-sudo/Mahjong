const CACHE_VERSION = 'mahjong-soul-p2p-v12-2026-08-28';
const APP_SHELL = [
...
self.addEventListener('fetch', event => {
  ...
  if(req.mode==='navigate'){
    event.respondWith(fetch(req).then(res=>{const cp=res.clone();caches.open(CACHE_VERSION).then(c=>c.put(req,cp));return res;}).catch(()=>caches.match(req).then(x=>x||caches.match('./'))));
    return;
  }
