...
export class MahjongRoom extends DurableObject {
  ...
  broadcastChat(ws, meta, msg) {
    ...
    // -----------------------------------------------------------------------
    //
    // Balasan AI hanya boleh dikirim oleh HOST.
    // Host akan mengirim aiSeat + nama + avatar + text.
    // Worker kemudian broadcast ke semua client.
    // -----------------------------------------------------------------------
    ...
    if (isAi) {
      ...
      if (
        !Number.isInteger(aiSeat) ||
        aiSeat < 0 ||
        aiSeat >= MAX_PLAYERS ||
        aiSeat === this.hostSeat
      ) {
        return;
      }
    ...
    //
    // FIX:
    // Sebelumnya seluruh object `msg` dikirim ke cleanChatText(),
    // sehingga JavaScript mengubahnya menjadi "[object Object]".
    //
    // Sekarang yang dibaca adalah msg.text.
    // -----------------------------------------------------------------------
