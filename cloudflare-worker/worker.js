...
const AVATAR_IDS = new Set(["panda", "shiba", "kucing", "penguin", "rubah"]);
function cleanAvatar(avatar) {
  const a = String(avatar || "").trim().toLowerCase();
  return AVATAR_IDS.has(a) ? a : "panda";
}
...
export class MahjongRoom extends DurableObject {
  constructor(ctx, env) {
    ...
    // players[seat] = { id, token, name, avatar, ready, connected, disconnectedAt }
    this.players = [null, null, null, null];
    // ws -> { seat, id, role, name, avatar, token }
    ...
    this.loading = (async () => {
      try {
        ...
        if (saved && typeof saved === "object") {
          this.roomCode = saved.roomCode || this.roomCode;
          this.hostSeat = Number.isInteger(saved.hostSeat) ? saved.hostSeat : HOST_SEAT;
          this.players = Array.isArray(saved.players) && saved.players.length === MAX_PLAYERS
            ? saved.players.map((p) => p && typeof p === "object" ? {
                id: p.id || "", token: cleanToken(p.token), name: cleanName(p.name), avatar: cleanAvatar(p.avatar),
                ready: !!p.ready, connected: false, disconnectedAt: Number(p.disconnectedAt) || null
              } : null)
            : [null, null, null, null];
          this.lastState = saved.lastState && typeof saved.lastState === "object" ? saved.lastState : null;
  ...
  async fetch(request) {
    ...
    this.touch();
    this.connections.set(server, { seat: null, id: clientId, role, name, avatar: "panda", token: null });
    ...
    server.addEventListener("close", () => { Promise.resolve(this.detach(server, false)).catch((err) => console.error(err)); });
    server.addEventListener("error", () => { Promise.resolve(this.detach(server, false)).catch((err) => console.error(err)); });
  ...
  async createRoom(ws, meta, msg) {
    ...
    const avatar = cleanAvatar(msg.avatar || meta.avatar);
    const token = cleanToken(msg.token) || crypto.randomUUID();
    if (meta.seat !== null && meta.seat !== this.hostSeat) this.players[meta.seat] = null;
    this.players[this.hostSeat] = { id: meta.id, name, avatar, ready: true, token, connected: true, disconnectedAt: null };
  ...
  async joinRoom(ws, meta, msg) {
    ...
    const avatar = cleanAvatar(msg.avatar || meta.avatar);
    const token = cleanToken(msg.token) || crypto.randomUUID();
    this.players[seat] = { id: meta.id, name, avatar, ready: true, token, connected: true, disconnectedAt: null };
    meta.seat = seat;
    meta.role = "guest";
    meta.name = name;
    meta.avatar = avatar;
  ...
  async rejoin(ws, meta, msg) {
    ...
    const avatar = cleanAvatar(msg.avatar || existing.avatar);
  ...
  roomView() {
    return {
      ...
      players: this.players.map((p) =>
        p ? { id: p.id, name: p.name, avatar: cleanAvatar(p.avatar), ready: !!p.ready, connected: p.connected !== false } : null
      ),
    };
  }
  ...
  broadcast(msg, except = null) {
    ...
    for (const [ws] of this.connections) {
      ...
      try {
        ...
        Promise.resolve(this.detach(ws, false)).catch((err) => console.error(err));
      }
    }
  }
  ...
  async scheduleCleanup() {
    try {
      await this.ctx.storage.setAlarm(Date.now() + SEAT_GRACE_MS);
    } catch (err) {
      console.error("Unable to schedule room alarm", err);
    }
  }
  ...
  async alarm() {
    ...
    if (changed) {
      this.touch();
      await this.persist();
      if (this.connections.size > 0) this.broadcastLobby();
    }
    await this.scheduleCleanup();
  }
}
